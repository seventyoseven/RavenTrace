...
class PortScanner:
    ...
    def _format_results(self) -> str:
        ...
        """
        lines = ["RavenTrace Port Scanner Results", "=" * 30, ""]
        
        # Add timestamp
        lines.append(f"Scan completed: {time.strftime('%Y-%m-%d %H:%M:%S')}")
        lines.append(f"Scan type: {self.config.scan_type.name}")
        lines.append("")
        
        # Sort hosts by IP address
        sorted_hosts = sorted(self.results.keys(), 
                             key=lambda ip: [int(octet) for octet in ip.split('.')])
        
        for host in sorted_hosts:
            host_results = self.results[host]
            open_ports = [port for port, is_open in host_results.items() if is_open]
        ...
        hosts_with_open_ports = sum(1 for host, ports in self.results.items() 
                                   if any(is_open for port, is_open in ports.items()))
        
        lines.append(f"Summary: Scanned {total_hosts} hosts, {hosts_with_open_ports} with open ports")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="RavenTrace Port Scanner")
    
    # Target specification
    target_group = parser.add_mutually_exclusive_group(required=True)
    target_group.add_argument("-t", "--target", help="Target host to scan")
    target_group.add_argument("-n", "--network", help="Target network to scan (CIDR notation)")
    
    # Port specification
    port_group = parser.add_mutually_exclusive_group()
    port_group.add_argument("-p", "--ports", help="Ports to scan (comma-separated or range, e.g., '80,443,8000-8100')")
    port_group.add_argument("-c", "--common", action="store_true", help="Scan common ports only")
    
    # Scan options
    parser.add_argument("-s", "--scan-type", choices=["connect", "syn", "null", "fin", "xmas"],
    ...
    parser.add_argument("--delay", type=float, default=0.0,
                       help="Delay between port scans in seconds (default: 0.0)")
    parser.add_argument("--no-random", action="store_true",
                       help="Disable randomization of ports and hosts")
...
def parse_ports(ports_str: str) -> List[int]:
    """
    Parse ports from a string specification.
    
    Args:
        ports_str: String specification of ports (e.g., "80,443,8000-8100")
...
def main():
    ...
    config = ScanConfig(
        scan_type=scan_type_map[args.scan_type],
        timeout=args.timeout,
        max_threads=args.threads,
        delay=args.delay,
        randomize_ports=not args.no_random,
        randomize_hosts=not args.no_random,
        common_ports_only=args.common,
