...
class OpenVASIntegration:
    ...
    def save_report(self, report_id: str, filename: str, format: str = 'json') -> bool:
        ...
        Args:
            ...
            format: Output format ('json', 'xml', or 'txt')
        ...
        try:
            # Get report
            report_xml = self.get_report(report_id)
            if not report_xml:
                return False
            ...
            if format.lower() == 'xml':
                ...
                report_data = self.parse_report(report_xml)
                with open(output_path, 'w') as f:
                    json.dump(report_data, f, indent=2)
            elif format.lower() == 'txt':
                # Parse and save as text
                report_data = self.parse_report(report_xml)
                self._save_as_text(output_path, report_data)
            else:
    ...
    def _save_as_text(self, filename: Path, report_data: Dict) -> None:
        """
        ...
        with open(filename, 'w') as f:
            f.write("RavenTrace OpenVAS Scan Results\n")
            f.write("==============================\n\n")
            
            # Write scan information
            f.write(f"Scan start: {report_data.get('scan_start', 'Unknown')}\n")
            f.write(f"Scan end: {report_data.get('scan_end', 'Unknown')}\n")
            f.write(f"Total hosts: {len(report_data.get('hosts', []))}\n")
            f.write(f"Total vulnerabilities: {len(report_data.get('vulnerabilities', []))}\n\n")
            ...
            for host in report_data.get('hosts', []):
                f.write(f"Host: {host.get('ip', 'Unknown')}\n")
                
                if host.get('hostname'):
                    f.write(f"  Hostname: {host.get('hostname')}\n")
                    
                if host.get('os'):
                    f.write(f"  OS: {host.get('os')}\n")
                    
                if host.get('ports'):
                    f.write("  Open ports:\n")
                    for port in host.get('ports', []):
                        f.write(f"    {port.get('port', '')} ({port.get('protocol', '')}) - {port.get('service', '')}\n")
            ...
            for severity in ['10.0', '7.0', '4.0', '1.0', '0.0']:
                severity_vulns = [v for v in report_data.get('vulnerabilities', []) 
                                 if v.get('severity') == severity]
                
                if severity_vulns:
                    f.write(f"{severity_map.get(severity, 'Unknown')} Severity Vulnerabilities\n")
                    f.write(f"{'-' * 40}\n\n")
                    
                    for vuln in severity_vulns:
                        f.write(f"Name: {vuln.get('name', 'Unknown')}\n")
                        f.write(f"Host: {vuln.get('host', 'Unknown')}\n")
                        f.write(f"Port: {vuln.get('port', 'Unknown')}\n")
                        f.write(f"Severity: {severity_map.get(severity, 'Unknown')}\n")
                        f.write("\nDescription:\n")
                        f.write(f"{vuln.get('description', 'No description available')}\n\n")
                        f.write("Solution:\n")
                        f.write(f"{vuln.get('solution', 'No solution available')}\n\n")
                        f.write("-" * 80 + "\n\n")


def parse_arguments():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description="RavenTrace OpenVAS Integration")
    
    # Connection options
    parser.add_argument("--host", required=True, help="GVM host")
    parser.add_argument("--port", type=int, default=9390, help="GVM port (default: 9390)")
    parser.add_argument("--username", default="admin", help="GVM username (default: admin)")
    parser.add_argument("--password", required=True, help="GVM password")
    parser.add_argument("--no-verify", action="store_true", help="Disable SSL verification")
    
    # Scan options
    parser.add_argument("-t", "--targets", required=True, help="Comma-separated list of targets to scan")
    parser.add_argument("-n", "--name", help="Name for the scan")
    parser.add_argument("--config", help="Scan config ID (default: Full and fast)")
    parser.add_argument("--timeout", type=int, default=300, help="API timeout in seconds (default: 300)")
    parser.add_argument("--auto-delete", action="store_true", help="Auto-delete tasks after completion")
    
    # Output options
    parser.add_argument("-o", "--output", default="openvas_report", help="Output filename (without extension)")
    parser.add_argument("-f", "--format", choices=["json", "xml", "txt"], default="txt", help="Output format (default: txt)")
    
    return parser.parse_args()


def main():
    """Main function for the OpenVAS integration."""
    ...
    config = OpenVASConfig(
        gvm_host=args.host,
        gvm_port=args.port,
        gvm_username=args.username,
        gvm_password=args.password,
        verify_ssl=not args.no_verify,
        scan_config_id=args.config or "daba56c8-73ec-11df-a475-002264764cea",
    ...
    try:
        ...
        # Run scan
        report_id = integration.scan_hosts(targets, args.name)
        
        if report_id:
            # Save report
            output_filename = f"{args.output}.{args.format}"
            integration.save_report(report_id, output_filename, args.format)
        else:
